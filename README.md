...
> **仅运行在 Linux VPS（Debian/Ubuntu）上。** 一键脚本默认下载 `main` 源码并临时拉取 Go >= 1.20 构建，确保 `raw.githubusercontent.com/main/install.sh` 总是安装当前 main。需要走 Release 预编译包时显式设置 `RD_USE_RELEASE=1`。
...
```bash
curl -fsSL https://raw.githubusercontent.com/Boooob626/reality-deployer/main/install.sh | sudo env DEST=/opt/reality-deployer bash
curl -fsSL https://raw.githubusercontent.com/Boooob626/reality-deployer/main/install.sh | sudo env RD_USE_RELEASE=1 bash
curl -fsSL https://raw.githubusercontent.com/Boooob626/reality-deployer/main/install.sh | sudo env GO_BOOTSTRAP_VERSION=1.22.12 bash
```
...
> 开发者：从源码构建 Release tarball 用 `make package`。一键脚本默认跟随 `main`，设置 `RD_USE_RELEASE=1` 才使用预编译 Release 包。
...
- `download.angie.software ... trixie Release`: rerun the latest one-liner. It removes the old unversioned Angie apt source and writes Angie's versioned repo path, e.g. Debian 13 writes `/angie/debian/13 trixie main`.
