...
- **路由默认屏蔽 CN 站点**（`geosite:cn` + `geoip:cn` → block），可切 bypass-direct
...
curl -vk https://<domain>/                            # 返回 decoy 站点 + 有效证书
```
...
├── internal/
│   ├── wizard/              # 交互提示 + 校验 + 流程编排
│   ├── combo/               # VLESS-REALITY / VLESS-TLS / Hysteria2 inbound 生成
│   ├── reality/             # 目标源解析 + Apple 封禁 + X25519 密钥
│   ├── routing/             # block-CN / bypass / 广告 预设
